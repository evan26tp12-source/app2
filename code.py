...
def simulation(trafic):
    ...
    A chaque tour, l'avion en tête de liste atterrit,
    ...
    while len(file) > 0:
        ...
        # vérifier les crashs avant l'atterrissage
        crashs_ce_tour=[]
        for a in file:
            if a["fuel"] <= 0:
...
simulation(trafic)import random
trafic=[]
def generate_random_traffic(n, scenario="normal"):
    for i in range(n):
        avions = {}  # ← ici, un nouveau dictionnaire à chaque itération
