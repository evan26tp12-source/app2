...
def tri_selection_technique(trafic):
    ...
    for i in range(n - 1):
        vrai = i
        for j in range(i + 1, n):
            if policy_technical(trafic[j], trafic[vrai]) or \
               (trafic[j]["technical_issue"] == trafic[vrai]["technical_issue"] and \
                trafic[j]["fuel"] < trafic[vrai]["fuel"]):
                vrai = j
...
def tri_selection_medical(trafic):
    n = len(trafic)
    for i in range(n - 1):
        vrai = i
        for j in range(i + 1, n):
            if (policy_medical(trafic[j], trafic[vrai]) or
               (trafic[j]["medical"] == trafic[vrai]["medical"] and
                policy_technical(trafic[j], trafic[vrai])) or
               (trafic[j]["medical"] == trafic[vrai]["medical"] and
                trafic[j]["technical_issue"] == trafic[vrai]["technical_issue"] and
                trafic[j]["fuel"] < trafic[vrai]["fuel"])):
                vrai = j
...
def tri_selection_diplomatique(trafic): # on prend en paramètre la liste de tous les avion et l'ordre de priorite
    n = len(trafic)
    for i in range(0, n-1):
        min= i
        for j in range(i+1, n):
            if policy_diplomatique(trafic[j],trafic[min])==True:
                min = j
        trafic[i],trafic[min]=trafic[min],trafic[i]
    return trafic 
